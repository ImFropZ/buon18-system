...
import { roleColumns } from "@modules/setting/components/data-tables";
...
function onDeleteSelectedHandler(ids: number[]) {
  return systemAxiosInstance.delete(`/setting/roles`, { data: { ids } });
}
...
export function RoleDataTable() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ["roles", { limit, offset, "name:ilike": search }],
    queryFn: async () => {
      const response = await systemAxiosInstance.get(`/setting/roles`, {
    ...
    <div className="grid h-full grid-rows-[auto,1fr,auto] gap-2 pb-4">
      <div className="flex justify-end gap-4">
        ...
        <DeleteSelectButton
          ...
          table={table}
        />
        <Link href="/setting/roles/create">
